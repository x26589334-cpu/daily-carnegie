...
- `apply.html` — 개강 일정표·수강료 + **입학 상담 신청 폼** + 지도
...
## 외부 연동
- **상담 신청 폼**(`apply.html`): `<form data-sheet="">` — 비어 있으면 데모 모드(안내 메시지만). Apps Script 웹앱 URL을 넣으면 구글 시트 [데일카네기] 탭에 저장. 필드: name, phone, company, title, email, course, memo, agree.
- **전화 플로팅 버튼**: `main.js` 하단에서 생성, `tel:025560113`. 카톡 채널로 바꾸려면 여기서 href/문구 교체.
- **GA4 / 네이버 서치어드바이저**: 아직 미삽입. 삽입 시 각 페이지 `<head>`에 추가.

## 콘텐츠 출처 (2026-08 기준)
- 과정 정보(12주 커리큘럼, 특징, 특전, 시간, 수강료 4,000,000원, 서울 103기 2026.9.8~11.24 화 17:00~21:00, 문의 02-556-0113, 역삼로17길 47 카네기빌딩)는 carnegie.co.kr 공개 정보.
