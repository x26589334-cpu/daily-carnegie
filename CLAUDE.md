...
- ⚠️ **HTTPS 미적용**: GitHub Pages `https_enforced: false`이며 https 접속이 안 됩니다(인증서 미발급). 반면 모든 페이지의 canonical·og:url과 `sitemap.xml`·`robots.txt`는 `https://`로 적혀 있어 불일치 상태. Settings → Pages → Enforce HTTPS를 켠 뒤 검색엔진에 제출할 것.
