...
- [x] `their-interest` | 상대가 관심 있는 것으로 말문을 연다 | 원칙 8 상대의 관심사
- [x] `make-them-important` | 인정에 인색한 리더가 잃는 것 | 원칙 9 중요한 사람이라는 느낌
- [x] `avoid-argument` | 논쟁에서 이기고 사람을 잃는다 | 원칙 10 논쟁을 피한다
- [x] `never-say-wrong` | "그건 틀렸습니다"라고 말하기 전에 | 원칙 11 잘못을 지적하지 않는다
- [x] `admit-fault-first` | 잘못을 지적해야 할 때, 먼저 인정하고 시작하라 | 원칙 12 자기 잘못을 먼저 인정

## 2부 · 인간관계론 — 설득 (10편)
- [x] `friendly-start` | 협상은 첫 30초에 결정된다 | 우호적으로 시작한다
- [x] `yes-yes` | 반대하는 상대에게서 "예"를 끌어내는 순서 | 예 반응을 얻는다
- [x] `let-them-own` | 내 아이디어를 상대의 아이디어로 만드는 법 | 스스로 생각해냈다고 느끼게 한다
- [x] `see-their-side` | 왜 저 사람은 저렇게 생각할까 | 상대의 관점에서 본다
- [x] `sympathize` | 불만을 제기한 고객을 단골로 바꾸는 한 문장 | 공감한다
- [x] `noble-motives` | 사람은 자기가 괜찮은 사람이고 싶어 한다 | 고상한 동기에 호소한다
- [x] `dramatize` | 숫자만으로는 조직이 움직이지 않는다 | 극적으로 표현한다
- [x] `throw-challenge` | 유능한 직원이 지루해할 때 | 경쟁심을 자극한다
- [x] `praise-then-ask` | 지적 전에 칭찬, 그다음이 중요하다 | 칭찬으로 시작한다
- [x] `indirect-attention` | "그런데"라고 말하는 순간 칭찬은 사라진다 | 간접적으로 알린다
